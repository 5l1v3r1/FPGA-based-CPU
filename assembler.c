...
void printProgram(int *program,int limit){
    int i;
    for(i=0;i<limit;i++){
        printf("%d\t0x%x\n", i,*program);
        program++;
...
int main()
{
    ...
    char ch;
    int chch;
    int program[1000];//holds all instruntion
    ...
    if (fp != NULL)
    {
        ...
        while(fgets(line,sizeof line,fp)!= NULL)
        {
            ...
            while (token)
            {
                if (strcmp(token,"ldi")==0) //---------------LDI INSTRUCTION--------------------
                {
                    ...
                    op2 = strtok(NULL,"\n\t\r "); //getthe 2nd operand of ldi, which is the data that is to be loaded
                    ...
                    if ((op2[0]=='0')&&(op2[1]=='x')) //ifthe 2nd operand is twos complement hexadecimal
                        program[counter]=hex2int(op2+2)&0xffff; //convertit to integer and form the second 16-bit
                    else if (( (op2[0])=='-') || ((op2[0]>='0')&&(op2[0]<='9')))
                        //if the 2nd operand is decimal
                        program[counter]=atoi(op2)&0xffff; //convert it to integer and form the second 16-bit
                    else //if the second operand is not decimal or hexadecimal, it is a label or a variable.
                    { //in this case, the 2nd 16-bits of the ldi instruction cannot be generated.
                        ...
                        op1=(char*)malloc(sizeof(op2)); //andthe name of the label/variable that it must contain
                    ...
                    printf("%d\t0x%x\n", counter-1,program[counter-1]);
                    printf("%d\t0x%x\n", counter,program[counter]);
                    counter++; //skip to the next memory location
                }
                else if (strcmp(token,"ld")==0) //------------LD INSTRUCTION---------------------
                {
                    op1 = strtok(NULL,"\n\t\r "); //get the 1st operandof ld, which is the destination register
                    op2 = strtok(NULL,"\n\t\r "); //get the 2nd operandof ld, which is the source register
                ...
                {
                    //to be added
                    op1 = strtok(NULL,"\n\t\r "); //get the 1st operandof sd, which is the destination address source 2                 x
                    op2 = strtok(NULL,"\n\t\r "); //get the 2nd operandof sd, which is the source 1 register                            y
                    ch = (op1[0]-48) << 3 | ((op2[0]-48) << 6); //form bits 11-0 of machine code. 48 is ASCII value of '0'
                    program[counter]=0x3000+((ch)&0x00f8); //form the instruction and write it to memory
                    printf("%d\t0x%x\n", counter,program[counter]);
                    counter++; //skip to the next empty location in memory
                ...
                {
                    op1 = strtok(NULL,"\n\t\r "); //read the labeljumptable[noofjumps].location = counter; //write the jz instruction's location into the jumptable
                    ...
                    noofjumps++; //skip to the next empty locationin jumptable
            ...
            //for all jump/jz instructions
        {
            ...
            while ( strcmp(jumptable[i].label , labeltable[j].label) != 0 ){
                //if the label for this jump/jz does not match with the
        ...
        rewind(fp);
        while(fgets(line,sizeof line,fp)!= NULL) //skip till .data, if no .data, alsook.
        {
            token=strtok(line,"\n\t\r ");
